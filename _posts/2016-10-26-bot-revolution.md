...
팀마다 사용빈도는 다르지만 현재 모든 직원이 슬랙봇을 활용하고 있는데 지속적으로 업무 환경을 개선하는데 bot 기능이 상당한 기여를 하고 있다.
...
#### 상황 및 의도
서비스 내 용어가 팀별로 다르게 쓰이거나 여러가지로 불리고 있는 것들이 있었다. 혹은 서비스가 런칭/업데이트되면서 개편된 제품/기능이름들이 있었다. 이는 아는 사람끼리는 문제가 없지만 신규입사자나 아직 전달이 덜된 타팀과 소통할 때에는 오해가 생길 수 있을 수 있었다. 또한, 새로운 이름을 알리고 즉각 교정이 되는 방법이 필요했다. 이를 위해 UXD팀에서는 효율적인 방법을 고안하고자 했다.
...
시간이 지나고 제 목표를 달성한 슬랙봇들을 삭제하기까지 이르렀다.
...
# Case7. 슬랙 API를 활용한 데이터드리븐봇 (심화편)


#### 상황 및 의도
심화편. 지금까지의 팁과 다르게 좀 더 개발자 친화적인 방법.
부정 적립 알림, 체크인 경보기 발송 대상, 알아서 코인 결제, 연장계약 등
실제 유저의 행동에 대한 액션 중 주요하게 알아야 할 것을 봇으로 뽑아서 만든 것들 관련 챕터도 간단히나마 추가되어보면 어떨까요?
